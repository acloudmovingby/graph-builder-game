...
## Overview
This application is a single web page made for building graphs (as in graph theory, with nodes and edges). You can use your mouse to click and build graphs, and then export the graphs into a number of formats.
...
| File | Purpose |
|------|---------|
| `Main.scala` | App entry point; registers all EventListeners |
| `controller/Controller.scala` | Orchestrates components, holds mutable state |
| `controller/Event.scala` | All event definitions |
| `model/State.scala` | Application state + helper methods |
| `components/ops/DOMOps.scala` | RenderOp implementations |
| `components/maincanvas/` | Primary canvas rendering (nodes/edges) |
| `components/adjacencymatrix/` | Adjacency matrix view |
| `components/toolbar/` | Tool selection |

## Data Flow

The architecture maximizes testability by isolating logic into pure functions and pushing side-effects to the edges. The central pure function is `Controller.handleEventWithState`.

1. An **EventListener** captures a DOM event and creates an **Event** object (impure: reads DOM).
2. The `EventListener` dispatches the `Event` to **`Controller.handleEvent`**.
3. `Controller.handleEvent` calls the pure **`Controller.handleEventWithState`**.
4. `handleEventWithState` calls the pure `update` method on all **Components** → new **State**.
5. It calls the pure `view` method on all **Components** → `Seq[RenderOp]`.
6. The new `State` and `Seq[RenderOp]` are returned to `Controller.handleEvent`.
7. `Controller.handleEvent` saves the new `State` and calls `render()` on each `RenderOp`.
8. **`RenderOp.render()`** performs the final side-effect (impure: writes DOM).
