...
object MainCanvasComponent extends Component {
	override def update(state: State, event: Event): State = {
		event match {
			case MainCanvasMouseEvent(coords, eventType) =>
				val maybeHoveredNode = hoveringOnNode(coords, state.keyToData)
				val foo = eventType match {
					case Move =>
				...
				if (state.graph != foo.graph || state.toolState != foo.toolState || state.hoveringOnNode != foo.hoveringOnNode) {
					println(s"--------- $eventType ---------")
					...
					println(s"graph state: ${foo.graph}, tool state: ${foo.toolState}, hoveringOnNode=${foo.hoveringOnNode}")
				}
				foo
			case _ => state
		}
